...
const moviesAsObjects = movies.map((movie) => ({
  title: movie[0],
  year: Number(movie[1]),
  director: movie[2],
  duration: movie[3],
  genre: movie[4],
  rating: Number(movie[5]),
}));

console.log(moviesAsObjects);
...
function renderMovies(movies: Movie[]) {
  ...
  movies.forEach((movie) => {
    ...
    genre.innerText = `Genre: ${movie.genre.join(", ")}`;
    // # Number to String
...
function sortYearAscending() {
  // Kopie erstellen
  const sortedMovies = [...moviesAsObjects];
...
function filterMovies(searchTerm: string) {
  ...
  let filteredMovies = [...moviesAsObjects];
